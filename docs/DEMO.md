...
A fictional 3-page paper about an AI Reading Assistant / Visual Explainer Cards system.
Generated by ExplainLens itself (using PyMuPDF), with no copyright concerns.
...
python -m explainlens.cli analyze \
  --input examples/sample_paper.pdf \
  --output outputs/demo_pdf
```

**Open the result**:

```
outputs/demo_pdf/cards.html
```
