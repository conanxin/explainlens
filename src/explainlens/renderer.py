...
<section class="hero">
  ...
  <div class="hero-pills">
    <span class="pill">Input: {{ meta.input_type or 'text' }}</span>
    <span class="pill">Provider: {{ meta.provider or 'rule-based' }}</span>
    {% if meta.image_adapter %}
    <span class="pill">Image: {{ meta.image_adapter }}</span>
    {% else %}
    <span class="pill">No image adapter</span>
    {% endif %}
    <span class="pill">{{ cards|length }} cards</span>
    <span class="pill">{{ meta.chunk_count }} source chunks</span>
    <span class="pill">v0.2.0-alpha</span>
  </div>
</section>
...
<div class="summary-wrap">
  <div class="summary">
    ...
    <div class="summary-item">
      <div class="summary-label">Input Type</div>
      <div class="summary-value">{{ meta.input_type or 'text' }}</div>
    </div>
    <div class="summary-item">
      <div class="summary-label">Provider</div>
      <div class="summary-value">{{ meta.provider or 'rule-based' }}</div>
    </div>
    {% if meta.image_adapter %}
    <div class="summary-item">
      <div class="summary-label">Image Adapter</div>
      <div class="summary-value accent">{{ meta.image_adapter }}</div>
    </div>
    {% endif %}
    <div class="summary-item">
...
{% if meta.image_adapter %}
      <img src="images/{{ card.card_id }}.svg" alt="{{ card.title }}" style="width:100%;height:auto;display:block;" />
{% else %}
...
{% if meta.image_adapter %}
<div class="appendix-wrap">
  <div class="appendix-title">Image Manifest</div>
  <div class="appendix-item">
    <div class="appendix-item-header">
      <span class="appendix-chunk-id">Adapter: {{ meta.image_adapter }}</span>
      <span class="appendix-page">v{{ meta.image_adapter_version or '0.1' }}</span>
    </div>
    <div class="appendix-usedby">
      All images generated locally. No external image APIs were called.
      Image files are in the <code>images/</code> directory.
    </div>
  </div>
</div>
{% endif %}
...
<!-- Safety Boundary -->
<div class="appendix-wrap">
  <div class="appendix-title">Safety</div>
  <div class="appendix-item" style="border-left: 4px solid #22c55e;">
    <div style="font-size:0.85rem; line-height:1.7; color:#166534;">
      <strong>&#10003; No external image API</strong> &mdash; All images are local SVG placeholders.<br>
      <strong>&#10003; No external provider unless opted-in</strong> &mdash; Default provider is rule-based (offline).<br>
      <strong>&#10003; No document upload</strong> &mdash; Source text stays on your machine.<br>
      <strong>&#10003; No API keys in output</strong> &mdash; No secrets appear in generated files.
    </div>
  </div>
</div>
...
<footer class="footer">
  <div>
    <span class="safety-note">{% if meta.image_adapter %}Image adapter: {{ meta.image_adapter }} | External image API: {% if meta.uses_external_image_api %}yes{% else %}no{% endif %}{% else %}This local MVP does not call external APIs.{% endif %}</span>
  </div>
  <p>Generated by <a href="https://github.com/conanxin/explainlens" target="_blank">ExplainLens</a> v0.2.0-alpha &mdash; AI Teaching Director</p>
</footer>
...
def render_cards_html(
    ...
    image_adapter_version: Optional[str] = None,
    uses_external_image_api: bool = False,
    ...
    Args:
        ...
        image_adapter: Name of image adapter used (None if images skipped).
        image_adapter_version: Version of image adapter.
        uses_external_image_api: Whether external image API was called.
        provider: Name of the analysis provider used.
        input_type: Input source type (txt, md, or pdf).
    ...
    meta = {
        ...
        "uses_external_image_api": uses_external_image_api,
        "image_adapter_version": image_adapter_version,
        "provider": provider,
